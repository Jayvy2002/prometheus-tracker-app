...
Le trou : le panneau affichait une proposition générique sans décision
humaine. Brancher le panneau sur Solo/Coach apply aurait fait hériter à un
signal B le brouillon de A — le même trou que P2.4 lecture.
...
la revue. Les apply durables restent sur leurs surfaces.
...
→ idempotent sur la même semaine de revue (clé serveur
  watch-decide:{signal}:{decision}:{week_start})
...
→ revue wait / confiance low / signal clos → no_current_proposal
```
