...
| Marketplace | PR 75 ajoute `/coaches`, `/coaches/:coachId`, `/coach/profile`, `/coaching-requests` et un candidat SQL ; non livré | Premier parcours par filtres exacts, profil opt-in et demande privée. Le matching guidé, la capacité réelle, la modération et l’activation restent requis par la cible. |
...
| Historique technique | Lock contient questionnaire `20260911235551`, tandis que MIGRATIONS décrit encore un ancien état de 98 versions | Le lock et le replay font foi ; éviter de prendre un résumé daté pour un inventaire live. Actualiser la doc technique avec une nouvelle vérification dédiée. |
...
| M6 Engagement et activation | Acceptation, partage, conditions, paiement selon décisions, activation commune, résumé et complément | Tests sans faux paiement, refus d’un second lien, réservation/rejeu/expiration, récupération après échec externe. |
...
Le chantier Billing demeure conditionné aux décisions de prix, prestataire du service, commission, reversement, conditions et responsabilités. Sa partie engagement/paiement devient une dépendance du parcours marketplace payant, pas une raison de retarder les profils et la recherche testables.
