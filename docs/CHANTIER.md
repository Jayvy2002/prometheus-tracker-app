...
**Mis à jour : 15 septembre 2026.** Lots 1–16 + 16f–16g + **17–21a** dans `new-JV`. File ouverte : **lot 21b** (gros composants) puis **21c** (`coachingStore` + façade). Lots 22–23 après. Une CI verte ne clôt pas une ligne UX.

| **Lot ouvert : 21b**. Lots **17–21a** dans `new-JV`. Lots 11–16 : apply prod + parcours encore dus. Lots 2–10 : **2 Terminé** ; **3–10 Partiel**.
...
| **21** | **Découper les mini-apps** (ARCH03, ARCH04) | **21a Terminé** ; 21b–c à construire | 21a `App.tsx` → router / guards / bootstrap. 21b gros composants ; 21c `coachingStore` **avec façade**. |
...
| 21 | **21a livré** (`app/router`, `app/guards`, `app/bootstrap`). Reste : `stores/coachingStore.ts` ~89 KB ; `ClientDetailPage`, `ExerciseCard`, `Dashboard`, `ProgramSessionEditor`, `workoutStore`. |
