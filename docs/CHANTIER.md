...
1. **Edges — redéployées via Supabase Management API** (connexion authentifiée ChatGPT, **pas** une preuve CLI, pas de `SUPABASE_ACCESS_TOKEN` transmis) :
   ...
   - `coach-agent` **v24 → v25 → v26**, `ACTIVE`, `verify_jwt=true`, `import_map` `deno.json`, SHA live `5a8ee074df72b2858a12ad99befb74ee650cccdc0491047ce53f51ba22748cd3`. v26 conserve le bundle métier audité et corrige uniquement l’encodage UTF-8 du wrapper Management API.
   - Smokes non destructifs : fleet OPTIONS **200** + POST sans JWT / JWT invalide / faux `X-Webhook-Key` → **401** `unauthorized`. `coach-agent` v26 : OPTIONS **200** avec CORS ; POST sans JWT → **401** gateway `UNAUTHORIZED_NO_AUTH_HEADER`.
   - **Wrapper Management API corrigé en live** : v25 échouait sur `btoa()` avec le source Unicode FR ; v26 convertit le source en octets UTF-8 avant Base64. Le preflight navigateur est rétabli. Ce correctif concerne seulement l’enveloppe de chargement live, pas la logique métier Git. PR **#69** reste un workflow CLI manuel optionnel, en **draft**, à ne pas merger tant que le secret repo et le workflow n’ont pas été validés.
2. **Rappels cron — actif** : un job `send-daily-reminders` actif (`* * * * *`) et le secret Vault `REMINDERS_CRON_SECRET` ont été vérifiés en production ; les cinq dernières exécutions contrôlées sont `succeeded`. Les secrets Edge/VAPID ont été configurés côté ops mais leurs valeurs ne sont pas lisibles par ce connecteur.
...
- Edges : `coach-agent` **v26** (bundle métier audité, wrapper UTF-8 corrigé, worker/CORS OK) ; `coach-fleet-round` **v32** (SHA audité, worker OK) ; `notify-onboarding-complete` **v21** ; `analyze-product` **v14** ; `verify-exercise` **v12** ; `delete-account` **v8** ; `send-daily-reminders` **v10** ; `batch-verify-exercises` **v7** (410).
