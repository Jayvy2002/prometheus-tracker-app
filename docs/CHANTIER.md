...
> **CURRENT IMPLEMENTATION GATE — P5, passage d’audit, audits 2 et 3 (`p5-20` à `p5-30`) et P6.1 mergés et déployés en production ; prochaine étape : P6.2 — bêta bypass, sur une branche dédiée et uniquement avec le feu vert de Jean-Vincent.** Production/lock **158** (dernière version `20260925110000_start_workout_catalog_link`), vérifié en production le 26 septembre 2026 après le merge de `#248` : Git, lock et production identiques. `supabase/migrations.pending.json` est vide. Secret `ACCOUNT_PURGE_CRON_SECRET` en place le 26 septembre (vault et secrets Edge, même valeur) : appel de purge authentifié accepté par `delete-account` (200, rien à purger). Aucune action manuelle restante. Jobs actifs : `coach-import-preview-purge` (`15 * * * *`), `account-deletion-purge` (`40 * * * *`). Watch n’applique pas.
>
...
P1.5–P2.5, P3, P4 (`#210`), P5.1 (`#213`), P5.2 (`#215`, `#216`), P5.3 (`#217`), P5.4 (`#219`), le passage d’audit (`#221`), l’alignement Vision (`#223`), l’audit 2 (`#224`–`#236`), P6.1 (`#222`) et l’audit 3 (`#238`–`#248`, `#252`) sont en production (lock 158). Un merge sur `new-JV` déploie migrations et Edge Functions (intégration GitHub Supabase). Watch n’applique pas. Prochaine sous-tâche : P6.2, avec feu vert.
...
5. **Fait le 26 septembre** (vérifié : vault présent, appel authentifié accepté) : créer `ACCOUNT_PURGE_CRON_SECRET` avec la même valeur dans le vault (`vault.create_secret`) et dans les secrets Edge. Sans lui, les suppressions demandées restent en attente, rien n’est supprimé, et le job `account-deletion-purge` échoue de façon visible.
...
**Technique** — (`start_workout_from_template` porte `catalog_exercise_id` avec `p5-30`) ; remplacement global des couleurs par les tokens du design system (sans bénéfice fonctionnel, non prioritaire).

## Audit 3 — ergonomie frontend (`p5-20` à `p5-30` et `p5-34` mergées)
...
|---|---|---|
| `p5-20` Confiance et langage | Un démarrage refusé le dit toujours ; icône « modifier » du poids ; « Annuler » une suppression de séance ne ment plus (une séance de programme se supprime après confirmation, sans annulation impossible) ; demandes de coaching vues du bon côté (Coach / athlète) ; bouton + seulement là où il sert (jamais sur un formulaire ni une page qui a son action) ; stats sans verdict sur la journée en cours ni sur moins de 3 jours, écarts en neutre ; bilan 2 semaines lisible ; types de série en français (É = échauffement) ; kcal et P / G / L partout ; une seule IA nommée Prometheus, icône toujours légendée ; jargon remplacé (logs, Perf, Stagnation, Adhérence, déficit calorique…) ; axe du poids en nombres ronds | Mergé (PR `#238`) |
| `p5-21` Séance guidée | Cases pré-remplies par la dernière séance puis la prescription (jamais « 0 », un tap n'enregistre qu'un vrai nombre) ; badge « 4 × 10 · repos 1 min 30 · RIR 0 » ; colonne Durée pour les séries isométriques (seul signal fiable, jamais le nom) ; bouton « Disques » explicite ; noms sur deux lignes ; légende compacte ; page Entraînement : prochaine routine du Solo avec Démarrer, explication des routines, dernière séance repliée, historique sans tonnage | Mergé (PR `#239`) |
| `p5-22` Aujourd'hui d'abord et navigation | Dashboard mobile : action du jour, puis la journée, puis « À regarder » ; cartes asynchrones révélées ensemble (plus de saut) ; bilan 2 semaines replié s'il n'attend pas de décision ; Solo sans routine guidé ; menus mobile et desktop identiques (Corps : Nutrition, Poids, Mensurations, Check-in, Photos ; Suivi : Calendrier, Exercices, Résumé) ; sous-onglets gardés sur les liens directs, défilement signalé ; avatar Profil sur les pages du Coaché | Mergé (PR `#240`) |
| `p5-23` Formulaires | Check-in : curseurs « non répondu » sans faux zéro, effaçables ; heures de sommeil « ex. 7,5 » en h (7,5 / 7h30) ; Prometheus en action secondaire. Nutrition : « Réutiliser » explicite, boutons nommés de 44 px, succès seulement après écriture. Onboarding : prénom d'abord, « il manque : … » annoncé | Mergé (PR `#241`) |
| `p5-24` Coach | File du jour : messages non lus et check-ins à relire ajoutés aux décisions, séances prévues en contexte, erreur avec Réessayer ; objectif client d'une seule source ; fiche mobile lisible (nom, onglets signalés, dossier complet expliqué) ; éditeur compact (résumé « 4 × 10 · repos 90 s », menu « … », superset « Enchaîner avec… » sur les mêmes valeurs) ; correction : l'éditeur perdait type de série, superset, catalogue et paramètres drop/tempo/cluster/myo à l'ouverture ; statut Brouillon / Enregistré / Actif · version · clients ; Prometheus Coach : suggestions par client ; profil coach : barre Enregistrer fixe, tarif « 120,50 » en centimes exacts | Mergé (PR `#242`) |
| `p5-25` Accessibilité et performance | Boutons icône nommés et de 44 px (routines, scanner, invitations, chrono, logger…) ; gris lisibles sur fond noir (neutral-500 ≈ 6,9:1, neutral-600 ≈ 5,7:1, bordures inchangées) ; plus de texte à 9 px (10 → 11 px) ; mouvement réduit déjà global (vérifié et verrouillé par un test) ; seul le français est dans le bundle principal, l'anglais se charge à la demande (bundle 1,46 → 1,34 Mo, 444 → 408 Ko compressé) | Mergé (PR `#243`) |
| `p5-26` Objectif performance à l'accueil | L'onboarding propose « Performance » : l'objectif est démarré dans le cycle (`start_goal`) avant l'enregistrement du profil, qui garde « maintenir » comme base énergétique (aucun déficit ni surplus inventé). Migration `20260925090000_performance_goal_energy_basis` : écrire cette base dans le profil n'est plus un nouvel objectif ; les autres changements d'objectif du profil restent enregistrés. Test `performance_goal_energy_basis.sql` (CI) | Mergé (PR `#244`) |
| `p5-27` Exercices chronométrés | Le catalogue dit comment un exercice se mesure (`measurement` : `reps` par défaut, `time` pour un maintien ; migration `20260925100000_exercise_measurement`, seul le gainage passe en `time`, Farmer Walk reste libre). Un exercice chronométré du catalogue démarre en séries isométriques (séance libre, routine, série ajoutée) et se prescrit en maintien de 30 s dans l'éditeur de programme, modifiable ; le badge n'affiche plus « × 10 » sur un maintien ; la durée affichée dans l'éditeur est celle enregistrée (elle restait vide). Rien de déjà enregistré ne change, le nom n'est jamais utilisé pour deviner. Test `exercise_measurement.sql` (CI) | Mergé (PR `#245`) |
| `p5-28` « Aujourd’hui » et dates | La page d’accueil s’appelle « Aujourd’hui » / « Today » partout (onglet, menu, titre de fenêtre, Dashboard Coach) ; l’en-tête ne répète plus le nom, seulement la date. Les 11 champs date passent par `DateField` : affichage et saisie dans la langue de l’app, saisie tolérante (`/`, `-`, `.`, 8 chiffres, ISO collé, `/` automatique sur pavé numérique), bouton calendrier natif de 44 px, erreur accessible sans jamais envoyer une date impossible ou hors bornes ; la date de séance du logger aussi (`DateInput` s’appuie dessus). Un test interdit tout nouveau `type="date"` hors `DateField` | Mergé (PR `#246`) |
| `p5-29` Thème clair | Profil › Unités & préférences : Sombre (défaut) / Clair / Automatique (suit le téléphone). Couleurs par variables CSS (`src/shared/theme/palette.ts`, une source pour Tailwind et les graphiques) : le sombre est identique au pixel près, le clair tient AA (texte principal 16,4:1, gris secondaires ≥ 5,3:1, accents foncés ≥ 4,5:1, texte blanc gardé sur les fonds pleins). Thème posé avant le premier rendu (pas de flash), barre de navigateur et d’état suivies. Limites : écran de lancement PWA noir, `color-scheme` natif laissé tel quel en sombre | Mergé (PR `#247`) |
| `p5-30` Noms d’exercices dans la langue de l’app | Un exercice du catalogue s’affiche dans la langue de l’app (« Gainage » en FR, « Plank » en EN) partout où un nom d’exercice apparaît : séance, récap, résumé de fin, historique et Progression (la recherche trouve aussi le nom affiché), routines, éditeur et programmes, fiche client, graphique de charge, propositions Prometheus, contraintes. Affichage seul (`exerciseDisplayName`, `useExerciseDisplayName`) : le nom enregistré ne change pas (historique et records s’y appuient), un nom écrit ou renommé reste tel quel. Migration `20260925110000_start_workout_catalog_link` : une séance démarrée d’une routine ou d’un programme garde le lien catalogue de la ligne (le déclencheur P5.3 ne le retrouvait que par le nom, donc pas pour un exercice renommé) ; un id envoyé par le client n’est gardé que pour une ligne lisible (vérifiée ou à soi). Test `start_workout_catalog_link.sql` (CI) | Mergé (PR `#248`) |
| `p5-34` Finitions de l’audit 3 | Calendrier natif, listes, cases et barres de défilement sombres dans le thème sombre (`color-scheme` déclaré dans les deux thèmes) ; logo blanc assombri en clair partout (démarrage, menu desktop, connexion, invitations, erreurs) ; page hors ligne au thème de l’app (cache `prometheus-v5`) ; le champ nom de l’éditeur de programme montre « Gainage » et enregistre ce qui est tapé. L’écran de lancement du système reste noir (`manifest.json` est fixe, il ne peut pas suivre un choix fait dans l’app). Test sur vrai téléphone à faire à la main (voir ci-dessous) | Mergé (PR `#252`) |

À vérifier sur un vrai téléphone après le merge de `p5-34` (non testable ici, branchement vérifié en navigateur) :

1. iPhone, app installée : passer en thème Clair, fermer complètement l’app puis la rouvrir ; l’heure et la batterie doivent être en noir sur fond clair.
2. Un formulaire avec une date (Mensurations par exemple) : toucher l’icône calendrier ; le calendrier du téléphone doit s’ouvrir, et la date choisie doit s’afficher en jj/mm/aaaa.
...
- Le thème clair est un choix, pas le défaut : l’app reste sombre tant que la personne ne choisit pas Clair ou Automatique (`p5-29`).
- `start_workout_from_template` porte `catalog_exercise_id` (`p5-30`) : le lien choisi dans la routine ou le programme fait foi ; la résolution par nom du déclencheur P5.3 reste le repli pour ce qui n’a pas de lien.
