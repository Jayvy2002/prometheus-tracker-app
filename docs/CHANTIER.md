...
> **CURRENT IMPLEMENTATION GATE — P5, passage d’audit, audits 2 et 3 et P6.1 mergés et déployés en production ; `p5-30` (noms d’exercices dans la langue de l’app, PR `#248`) en revue, à ne fusionner qu’avec le feu vert de Jean-Vincent ; ensuite P6.2 — bêta bypass, sur une branche dédiée et uniquement avec le feu vert de Jean-Vincent.** Production/lock **157** (dernière version `20260925100000_exercise_measurement`), vérifié en production le 25 septembre 2026 après les merges `#237` à `#247` : Git, lock et production identiques. `supabase/migrations.pending.json` est vide ; `p5-30` y ajoutera `20260925110000_start_workout_catalog_link`. Action manuelle restante : le secret `ACCOUNT_PURGE_CRON_SECRET` (voir « Déploiement de l’audit 2 et de P6.1 »). Jobs actifs : `coach-import-preview-purge` (`15 * * * *`), `account-deletion-purge` (`40 * * * *`). Watch n’applique pas.
>
...
## Audit 3 — ergonomie frontend (`p5-20` à `p5-29` mergées, `p5-30` en revue)
