...
**M1 — contexte de compte, partiel.** Les routes principales et helpers partagent une projection compatible de la capacité professionnelle, de l’accompagnement personnel et de l’espace par défaut. Les lectures et modifications de rôle sont liées au compte et invalidées au reset ; les réponses remplacées sont ignorées, les modifications concurrentes locales sont bloquées et seul un rôle serveur reconnu est mémorisé. Cette projection frontend ne remplace pas une autorisation serveur.

Restent : inventaire exhaustif des prédicats, capacités persistées et backfill, contexte serveur, choix Personnel/Coaching, accès personnel du coach, mutations entre appareils et validation des parcours de changement de compte. Les autres opérations du store doivent être auditées individuellement.

**M2 — départ, en validation.** Les deux initiateurs passent par la même transition SQL, qui verrouille le lien actif avant les écritures. Le départ met le programme en pause, conserve l’historique et retire le suivi du coach. La RPC cliente et la table de notifications restent dans le SQL candidat appliqué uniquement à la base temporaire de CI. Aucune migration de ce lot n’est déployée.

Les tests SQL du [run 34714511819](https://github.com/Jayvy2002/prometheus-tracker-app/actions/runs/34714511819) passent pour l’isolation, les droits anonymes, la répétition, l’annulation complète après erreur injectée, les archives lisibles par le client et le retrait de l’accès à son historique pour l’ancien coach. Cette preuve précède l’ajout des notifications.

La branche ajoute ensuite : isolation des lectures coach/profil/réglages et de la mutation de départ ; succès distinct du rafraîchissement de profil ; bannière de fin neutre ; notification durable réservée à l’ancien coach avec accusé de lecture ; suppression du repli frontend qui pouvait exécuter partiellement un départ initié par le coach. Le scénario navigateur inclut une panne de rafraîchissement après départ, le retour solo et la lecture de la notification. La concurrence de deux départs et la conservation de la capacité professionnelle sont ajoutées aux tests. **Validation partielle :** le SQL étendu (notification privée, accusé idempotent et capacité professionnelle conservée) passe dans le [run 34715346015](https://github.com/Jayvy2002/prometheus-tracker-app/actions/runs/34715346015), avec la matrice historique. Le parcours navigateur a révélé deux barrières d’onboarding lors du retour solo ; elles sont corrigées sans marquer artificiellement le questionnaire comme rempli. Le test exige maintenant l’accueil immédiatement après le départ malgré une panne de rafraîchissement, puis après rechargement. Il exerce aussi le vrai store avec une RPC retardée et un changement de compte.

Le [run 34715507710](https://github.com/Jayvy2002/prometheus-tracker-app/actions/runs/34715507710), sur le correctif `3a9125f`, est encore en attente d’un runner au dernier contrôle. Ne pas annoncer le parcours final réussi à partir des runs précédents : leurs scénarios navigateur échouaient avant ce correctif. Huit tests d’isolation de requêtes/mutations passent localement. La CLI 2.117.0 fonctionne dans l’environnement de travail ; Docker y est absent, donc le replay Supabase complet reste réalisé en CI. Le lock conserve uniquement les versions réellement appliquées en production.

Restent avant livraison de M2 : valider les nouvelles preuves SQL/navigateur, départ contre adaptation ou nouvelle activation, changements de compte pendant les opérations, accords explicites de partage pour le prochain coach, révocation des caches et accès fichiers concernés, puis générer et promouvoir la migration. La bannière n’attribue plus par défaut le départ au coach ; l’auteur/date de fin doit encore être tracé explicitement dans le modèle de relation. Une notification dans l’application ne constitue pas une notification push ou email.

Le profil public, l’annuaire, les demandes et les autres lots M0–M8 restent à construire ou compléter. Aucun critère de ces lots n’est retiré.
