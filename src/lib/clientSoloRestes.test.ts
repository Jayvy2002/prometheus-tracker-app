...
test('steps journal calls logSteps from Nutrition and Dashboard reads the log', () => {
  ...
  assert.doesNotMatch(nutrition, /showNutritionField\(tracking, 'steps'\)/);
  assert.doesNotMatch(nutrition, /<StepsTracker/);
  ...
  assert.doesNotMatch(dash, /showNutritionField\(tracking, 'steps'\)/);
