...
function getWeekAvg(measurements: { weight_kg: number; measured_at: string }[], daysAgo: number, windowDays: number): number | null {
  const now = new Date();
  ...
  return inWindow.reduce((s, m) => s + m.weight_kg, 0) / inWindow.length;
}
...
export default function WeeklyAdjustment({ onDismiss }: Props) {
  ...
  const avgThisWeek = getWeekAvg(measurements, 0, 7);
  const avgLastWeek = getWeekAvg(measurements, 7, 7);

  if (avgThisWeek === null || avgLastWeek === null) return null;

  const weekDiff = avgThisWeek - avgLastWeek;
  ...
  if (goal === 'cut') {
    if (absDiff < 0.2) {
      ...
      message = 'Weight barely moved this week.';
      reason = 'Reduce by 100 kcal to maintain deficit.';
      severity = 'warning';
    } else if (weekDiff < -1) {
      suggestion = currentCalories + 100;
      message = 'You\'re losing weight too fast (-' + absDiff.toFixed(1) + 'kg/week).';
      reason = 'Increase by 100 kcal to protect muscle.';
      severity = 'warning';
    } else if (weekDiff > 0.3) {
      suggestion = currentCalories - 150;
      message = 'Weight went up this week (+' + absDiff.toFixed(1) + 'kg).';
    ...
    if (weekDiff < 0.1) {
      suggestion = currentCalories + 100;
      message = 'Weight isn\'t increasing.';
      reason = 'Increase by 100 kcal to support muscle growth.';
      severity = 'info';
    } else if (weekDiff > 0.6) {
      suggestion = currentCalories - 100;
      message = 'Gaining too fast (+' + absDiff.toFixed(1) + 'kg/week).';
    ...
    if (absDiff > 0.5) {
      ...
      message = `Weight ${weekDiff > 0 ? 'increased' : 'decreased'} by ${absDiff.toFixed(1)}kg this week.`;
