  ...
  return (
    <main className="min-h-screen bg-black text-white p-6 flex items-center justify-center">
      <div className="max-w-lg w-full space-y-5">
        ...
        {(['solo', 'find_coach', 'coach'] as const).map(intent => (
          <Button
            key={intent}
            className="w-full justify-start text-left min-h-24"
            variant="secondary"
            disabled={busy}
            onClick={() => { void select(intent); }}
          >
            <span>
              <span className="block font-semibold">{t(`entryIntention.${intent}`)}</span>
              <span className="block text-sm text-neutral-400 font-normal mt-2">
                {t(`entryIntention.${intent}Hint`)}
              </span>
            </span>
          </Button>
        ))}
