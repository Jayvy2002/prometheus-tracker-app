...
export default function StepsWidget({ size }: { size: string }) {
  ...
  return (
    <div>
      <div className="flex items-end gap-2 mb-2">
        ...
        {editing ? (
          <div className="flex items-center gap-1 flex-1">
            ...
            />
            <button onClick={handleSave} className="text-xs text-blue-400 hover:text-blue-300 px-1 transition-colors">Save</button>
            <button onClick={() => setEditing(false)} className="text-xs text-neutral-500 hover:text-neutral-300 transition-colors">Cancel</button>
          </div>
        ) : (
          <>
            <button onClick={() => { setDraft(steps.toString()); setEditing(true); }} className="flex items-end gap-2 group">
              ...
              <span className="text-xs text-neutral-400 mb-0.5">/ {target.toLocaleString()} steps</span>
            </button>
          </>
        )}
      </div>
      ...
      <p className="text-xs text-neutral-500 mt-1">{Math.round(pct)}% of daily goal</p>
    </div>
  );
}
