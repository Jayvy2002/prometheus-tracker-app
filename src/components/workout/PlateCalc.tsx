    ...
    <Modal open={open} onClose={onClose} title={t('workout.plates.title')}>
      ...
      <div className="mb-4 grid grid-cols-3 gap-2 text-center">
        <div className="rounded-xl bg-neutral-900 px-2 py-2">
          <p className="text-[11px] uppercase tracking-wider text-neutral-500">{t('workout.plates.barLabel')}</p>
          <p className="text-sm font-semibold text-white tabular-nums">{bar} {unit}</p>
        </div>
        <div className="rounded-xl bg-neutral-900 px-2 py-2">
          <p className="text-[11px] uppercase tracking-wider text-neutral-500">{t('workout.plates.perSide')}</p>
        ...
        <div className="rounded-xl bg-blue-600/15 ring-1 ring-blue-500/30 px-2 py-2">
          <p className="text-[11px] uppercase tracking-wider text-blue-300/80">{t('workout.plates.total')}</p>
            ...
            sleeve.map((plate, index) => {
              ...
              return (
                <button
                  ...
                  className={`mx-px flex shrink-0 items-center justify-center rounded-[3px] text-[11px] font-bold ring-1 ${style.bg} ${style.text} ${style.ring}`}
