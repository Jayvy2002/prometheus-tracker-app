...
export default function CoachRelationshipNotices() {
  ...
  const [state, setState] = useState<{ owner?: string; rows: Notice[]; failed: boolean }>({ rows: [], failed: false });
    ...
    const load = async () => {
      ...
      try {
        const { data, error } = await supabase.from('coach_relationship_notices')
          .select('id,client_name,ended_at').eq('coach_id', userId).is('read_at', null)
  ...
  return <Card className="mb-6 space-y-3">
    <h2 className="text-sm font-semibold text-white">{t('coachDiscovery.notices.title')}</h2>
    {state.rows.map(row => <div key={row.id} className="flex items-center justify-between gap-3">
      <div className="min-w-0">
        <p className="text-sm text-neutral-300">{t('coachDiscovery.notices.ended', { name: row.client_name || t('coachDiscovery.notices.client') })}</p>
        <time className="text-xs text-neutral-500" dateTime={row.ended_at}>{new Date(row.ended_at).toLocaleDateString(i18n.language)}</time>
      </div>
      <Button variant="secondary" size="sm" loading={pending.includes(row.id)} onClick={() => void dismiss(row.id)}
        aria-label={t('coachDiscovery.notices.dismissFor', { name: row.client_name || t('coachDiscovery.notices.client') })}>
        {t('coachDiscovery.notices.dismiss')}
      </Button>
    </div>)}
    {state.failed && <div role="alert">
      <p className="text-sm text-amber-300">{t('coachDiscovery.notices.error')}</p>
      <Button variant="secondary" size="sm" onClick={() => setRevision(n => n + 1)}>{t('errors.retry')}</Button>
    </div>}
  </Card>;
}
