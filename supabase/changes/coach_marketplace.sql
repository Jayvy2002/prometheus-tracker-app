-- Candidate only. Apply to an isolated database after account_capabilities.sql.
CREATE TABLE IF NOT EXISTS public.coach_profiles (
 ...
 disciplines text[] NOT NULL DEFAULT '{}' CHECK (disciplines <@ ARRAY['strength','powerlifting','general_fitness']::text[] AND cardinality(disciplines) <= 3),
 languages text[] NOT NULL DEFAULT '{}' CHECK (languages <@ ARRAY['fr','en']::text[] AND cardinality(languages) <= 2),
 formats text[] NOT NULL DEFAULT '{}' CHECK (formats <@ ARRAY['online','in_person','hybrid']::text[] AND cardinality(formats) <= 3),
 area text NOT NULL DEFAULT '' CHECK (length(area) <= 150),
 published boolean NOT NULL DEFAULT false,
 accepting_clients boolean NOT NULL DEFAULT false,
 updated_at timestamptz NOT NULL DEFAULT clock_timestamp(),
 CHECK (NOT published OR (length(btrim(introduction)) > 0 AND length(btrim(method)) > 0 AND length(btrim(offer)) > 0
   AND cardinality(disciplines) > 0 AND cardinality(languages) > 0 AND cardinality(formats) > 0
   AND (NOT (formats && ARRAY['in_person','hybrid']::text[]) OR length(btrim(area)) > 0)))
);
...
-- Capability rows are private. A narrowly scoped helper checks public eligibility
-- without exposing other users' capability records through their RLS policy.
CREATE OR REPLACE FUNCTION public.marketplace_coach_eligible(p_coach uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = '' AS $$
 SELECT auth.uid() IS NOT NULL AND EXISTS (SELECT 1 FROM public.user_capabilities WHERE user_id=p_coach AND capability='coach')
 AND (p_coach=auth.uid() OR EXISTS(SELECT 1 FROM public.coach_profiles WHERE coach_id=p_coach AND published))
$$;
REVOKE ALL ON FUNCTION public.marketplace_coach_eligible(uuid) FROM PUBLIC, anon;
...
CREATE POLICY "marketplace_profile_read" ON public.coach_profiles FOR SELECT TO authenticated
 USING (coach_id = (select auth.uid()) OR (published AND public.marketplace_coach_eligible(coach_id)));

CREATE OR REPLACE FUNCTION public.save_my_coach_profile(p_profile jsonb, p_expected_updated_at timestamptz DEFAULT NULL)
RETURNS public.coach_profiles LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $$
DECLARE v_uid uuid := auth.uid(); v_previous public.coach_profiles; v_result public.coach_profiles;
BEGIN
 IF NOT public.marketplace_coach_eligible(v_uid) THEN RAISE EXCEPTION 'coach_required'; END IF;
 -- Serialize first creation as well as updates for the same account.
 PERFORM 1 FROM public.user_capabilities WHERE user_id=v_uid AND capability='coach' FOR UPDATE;
 ...
 END IF;
 IF v_previous.coach_id IS NULL AND p_expected_updated_at IS NOT NULL THEN RAISE EXCEPTION 'profile_changed'; END IF;
 INSERT INTO public.coach_profiles(coach_id,public_name,introduction,method,offer,disciplines,languages,formats,area,published,accepting_clients)
 VALUES(v_uid,btrim(p_profile->>'public_name'),coalesce(p_profile->>'introduction',''),coalesce(p_profile->>'method',''),coalesce(p_profile->>'offer',''),
 ARRAY(SELECT jsonb_array_elements_text(p_profile->'disciplines')),ARRAY(SELECT jsonb_array_elements_text(p_profile->'languages')),
 ARRAY(SELECT jsonb_array_elements_text(p_profile->'formats')),coalesce(p_profile->>'area',''),coalesce((p_profile->>'published')::boolean,false),coalesce((p_profile->>'accepting_clients')::boolean,false))
 ON CONFLICT(coach_id) DO UPDATE SET public_name=excluded.public_name,introduction=excluded.introduction,method=excluded.method,offer=excluded.offer,
 disciplines=excluded.disciplines,languages=excluded.languages,formats=excluded.formats,area=excluded.area,published=excluded.published,
 accepting_clients=excluded.accepting_clients,updated_at=clock_timestamp() RETURNING * INTO v_result;
 RETURN v_result;
END $$;
REVOKE ALL ON FUNCTION public.save_my_coach_profile(jsonb,timestamptz) FROM PUBLIC, anon;
...
BEGIN
 ...
 SELECT * INTO v_result FROM public.coach_join_requests WHERE id=p_request AND v_uid IN (coach_id,client_id) FOR UPDATE;
 IF NOT FOUND THEN RAISE EXCEPTION 'request_not_found'; END IF;
 IF NOT ((v_uid=v_result.client_id AND p_status='withdrawn') OR (v_uid=v_result.coach_id AND p_status IN ('accepted','declined'))) THEN
 RAISE EXCEPTION 'not_authorized'; END IF;
 IF v_result.status=p_status THEN RETURN v_result; END IF;
 IF v_result.status<>'pending' AND NOT (v_result.status='accepted' AND p_status='withdrawn') THEN RAISE EXCEPTION 'request_closed'; END IF;
 IF p_status='accepted' THEN
   PERFORM 1 FROM public.coach_profiles WHERE coach_id=v_uid AND published AND accepting_clients FOR SHARE;
   IF NOT FOUND OR NOT public.marketplace_coach_eligible(v_uid) THEN RAISE EXCEPTION 'coach_unavailable'; END IF;
   IF EXISTS(SELECT 1 FROM public.coach_client_links WHERE client_id=v_result.client_id AND status='active') THEN RAISE EXCEPTION 'already_coached'; END IF;
 END IF;
 UPDATE public.coach_join_requests SET status=p_status,updated_at=clock_timestamp() WHERE id=p_request RETURNING * INTO v_result;
 -- Agreement grants neither dossier access nor a paid subscription.
 RETURN v_result;
END $$;
REVOKE ALL ON FUNCTION public.respond_coaching_request(uuid,text) FROM PUBLIC, anon;
