-- M2b : consentement versionné sur l’invitation directe.
-- Ajoute accept_coach_invite(text, integer, text[]) sans révoquer l’overload (text).
-- La révocation de (text) est un lot suivant, après merge + déploiement du front.
...
COMMENT ON TABLE public.coaching_relationship_consents IS
  'Client consent for a direct invite. Inserts only via accept_coach_invite(text, integer, text[]).';
...
CREATE INDEX IF NOT EXISTS coaching_consents_coach_idx
  ON public.coaching_relationship_consents (coach_id, accepted_at DESC);
CREATE INDEX IF NOT EXISTS coaching_consents_client_idx
  ON public.coaching_relationship_consents (client_id, accepted_at DESC);
...
-- Nouvelle signature. L’overload (text) reste exécutable jusqu’à la révocation M2b-2.
...
BEGIN
  ...
  END IF;
  IF p_consent_version IS DISTINCT FROM 1 THEN
    RETURN jsonb_build_object('ok', false, 'error', 'consent_version_required');
  END IF;

  SELECT array_agg(DISTINCT scope ORDER BY scope)
  INTO v_normalized
  FROM unnest(p_scopes) AS item(scope);
...
AS $$
BEGIN
  UPDATE public.coaching_relationship_consents
...
CREATE TRIGGER revoke_relationship_consent
