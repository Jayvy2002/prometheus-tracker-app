-- Répare `triage_coach_fleet` : la migration 20260901004739 avait été écrite à partir
-- de la toute première version (20260829000006) et avait donc perdu deux itérations :
-- les clés `last_coach_message_at`, `last_keep_in_touch_at`, `pending_fleet`,
-- `fleet_handled` ainsi que la directive `#variable_conflict use_column`.
-- Sans ces clés, `idleDays(null, ...)` vaut +Infinity côté edge function : le
-- keep-in-touch et le cooldown de la tournée ne filtrent plus rien.
-- Cette migration reprend le corps du 20260829000010 et y garde l'apport du
-- 20260901004739 : la fréquence d'entraînement dérivée des jours du programme actif.
