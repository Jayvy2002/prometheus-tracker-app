-- Restore the fleet cooldown keys that 20260901004739 silently dropped.
--
-- 20260829000010 taught triage_coach_fleet to emit last_coach_message_at, last_keep_in_touch_at,
-- pending_fleet and fleet_handled. 20260901004739 then re-created the function from an older
-- copy to add program_frequency and lost those four keys. Production has run that version since:
-- coach-fleet-round reads them as null/false/[] so
--   - FLEET_HANDLE_COOLDOWN_DAYS never applies (a dismissed card comes back the next night),
--   - keep_in_touch is offered to every on-track client every night (coach silence = infinity),
--   - the upsert-vs-insert decision is always "insert" (the RPC's unique index is the only dedup).
--
-- This is the union of both versions: everything from 20260829000010 plus the program-derived
-- training_frequency from 20260901004739. coachFleet.test.ts now checks the LAST definition in
-- migration order against the Dossier the edge parses, so this cannot drift again.
