-- M1 : projection de capacité coach. user_roles reste la source d’écriture.
-- L’espace UI ne donne aucun privilège.
...
COMMENT ON TABLE public.user_capabilities IS
  'Compatibility projection of professional coach capability. Written only from user_roles.';
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    ...
    WHERE user_id = OLD.user_id AND capability = 'coach';
    RETURN OLD;
  END IF;
  ...
  ELSE
    DELETE FROM public.user_capabilities
    WHERE user_id = NEW.user_id AND capability = 'coach';
  END IF;
  RETURN NEW;
END;
$$;
...
CREATE TRIGGER sync_legacy_coach_capability
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.sync_legacy_coach_capability();

INSERT INTO public.user_capabilities (user_id, capability)
SELECT user_id, 'coach'
FROM public.user_roles
WHERE coaching_role = 'coach'
ON CONFLICT (user_id, capability) DO NOTHING;

-- One snapshot. No caller-controlled account id, no client dossier.
...
AS $$
  SELECT CASE
    ...
    ELSE jsonb_build_object(
      'user_id', auth.uid(),
      'coach_capability', EXISTS (
        SELECT 1 FROM public.user_capabilities
        WHERE user_id = auth.uid() AND capability = 'coach'
      ),
      'active_coach_id', (
        SELECT coach_id
      ...
      'legacy_coaching_role', COALESCE(
        (SELECT coaching_role FROM public.user_roles WHERE user_id = auth.uid()),
        'none'
      )
    )
  END;
...
COMMENT ON FUNCTION public.get_my_account_context() IS
  'Caller-scoped snapshot: coach capability and personal active coach. No client dossier. UI workspace is not an authorization source.';
