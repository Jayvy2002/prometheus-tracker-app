-- Audit hardening (lots D/E):
-- 1. user_profiles.timezone — device zone, used by reminders and (via coach_settings) the fleet.
-- 2. protect_coach_nutrition_targets also locks water / steps the coach set.
-- 3. Drop client-readable coach_notes (internal notes).
-- 4. Assigned clients can read a paused program (unlink keeps it).
-- 5. FK indexes the 2026-03 cleanup dropped and coaching FKs that never had one.
-- 6. triage_coach_fleet(p_client_id) so coach-agent does not re-aggregate the whole roster.
...
COMMENT ON COLUMN public.user_profiles.timezone IS
  'IANA timezone from the device (Profil bootstrap). NULL = treat stored times as UTC.';
...
BEGIN
  ...
  IF EXISTS (
    ...
    NEW.daily_water_target_ml := OLD.daily_water_target_ml;
    NEW.daily_steps_target := OLD.daily_steps_target;
  END IF;
  RETURN NEW;
END;
$$;
...
DROP POLICY IF EXISTS "Clients can read notes about them" ON public.coach_notes;
...
CREATE INDEX IF NOT EXISTS program_assignments_assigned_by_idx ON public.program_assignments (assigned_by);
CREATE INDEX IF NOT EXISTS workouts_program_assignment_id_idx ON public.workouts (program_assignment_id);
CREATE INDEX IF NOT EXISTS workouts_program_day_id_idx ON public.workouts (program_day_id);
CREATE INDEX IF NOT EXISTS program_days_routine_id_idx ON public.program_days (routine_id);
CREATE INDEX IF NOT EXISTS coach_notes_coach_id_idx ON public.coach_notes (coach_id);
CREATE INDEX IF NOT EXISTS coach_messages_sender_id_idx ON public.coach_messages (sender_id);
CREATE INDEX IF NOT EXISTS coach_agent_lessons_intervention_id_idx ON public.coach_agent_lessons (intervention_id);
...
COMMENT ON FUNCTION public.triage_coach_fleet(uuid, uuid) IS
  'Fleet triage for one coach, or one client when p_client_id is set (coach-agent).';
