...
BEGIN
  IF to_regclass('public.coach_imports') IS NOT NULL
     ...
     AND has_function_privilege('authenticated', 'public.list_coach_imports(timestamptz,uuid,integer)', 'execute')
...
BEGIN
  IF to_regclass('public.coach_provisional_dossiers') IS NOT NULL
     ...
     AND has_function_privilege('authenticated', 'public.confirm_provisional_claim(text,boolean,boolean,text,boolean)', 'execute')
     AND to_regprocedure('public.confirm_provisional_claim(text,boolean,boolean)') IS NULL
     AND has_function_privilege('authenticated', 'public.preview_coach_import(uuid,text,text,jsonb,text)', 'execute')
     ...
     AND NOT has_function_privilege('anon', 'public.confirm_provisional_claim(text,boolean,boolean,text,boolean)', 'execute')
...
BEGIN
  IF to_regclass('public.exercise_aliases') IS NOT NULL
     ...
     AND NOT has_function_privilege('authenticated', 'public.list_exercise_duplicate_candidates(integer,integer)', 'execute')
...
BEGIN
  IF to_regclass('public.platform_operators') IS NOT NULL
     ...
     AND has_function_privilege('authenticated', 'public.admin_list_open_reports(timestamptz,uuid,integer)', 'execute')
     ...
     AND NOT has_function_privilege('anon', 'public.admin_list_open_reports(timestamptz,uuid,integer)', 'execute')
